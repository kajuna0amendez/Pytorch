...
class MultiLayerPerceptronOptim(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, alpha,\
        ...
        self.epsilon = torch.tensor(epsilon, dtype=torch.float, device = cuda)
        self.gamma = torch.tensor(gamma, dtype=torch.float, device = cuda)
        self.beta = torch.tensor(beta, dtype=torch.float, device = cuda)
    ...
    def batch_normalization_layer(self, X):
        """
        the batch normalization for minbatches
        """
        bmean = torch.mean(X, 0)
        bvar  = torch.var(X,0)
        x_mbatch   = X - bmean/torch.sqrt(bvar+self.epsilon)
        
        Y = self.gamma(x_mbatch) + self.beta
    ...
    def forward(self, X):
        ...
        Forward pass
        """
        X_n = self.batch_normalization_layer(X)
        
        # Split first layer liner
        self.Y1 = torch.matmul(X_n, self.WIH)
    ...
    def backward(self, X, Y, Y4):
        ...
        # Change the grad data for using the opt
